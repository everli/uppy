          ...
          <tr class="border-t">
            ...
            <td>
              <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                <input type="checkbox" @change="updateForcedStatus($event, build)" :checked="build.forced" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer"/>
                <label :for="'forced-'+build.id" class="toggle-label block overflow-hidden h-6 rounded-full bg-gray-300 cursor-pointer"></label>
              </div>
            </td>
            <td class="py-2">
              <button type="button" @click="deleteBuild(build, index)" class="px-2 py-1 bg-red-800 text-white rounded hover:bg-red-600 focus:outline-none focus:bg-red-700 inline-flex items-center uppercase">
